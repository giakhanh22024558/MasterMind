...
| Script | Purpose |
|---|---|
| [`grey-out-non-scope.py`](grey-out-non-scope.py) | DFD canvas prep — keep in-scope cells visible + textual, grey out + empty everything else. Preserves structural style attrs (shape · container · dashed · rounded), only swaps color properties. |
