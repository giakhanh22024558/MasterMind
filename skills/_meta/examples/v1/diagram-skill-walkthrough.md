...
`diagram/` instantiates the abstract defer-then-promote pattern (from [`_meta/defer-then-promote-pattern/`](../../defer-then-promote-pattern/)) in two domain-specific tracking patterns:
...
| Patterns abstraction | CLR/HWG defer-then-promote codified | Pattern templates should have been more general from the start (refactored into `_meta/`) |
