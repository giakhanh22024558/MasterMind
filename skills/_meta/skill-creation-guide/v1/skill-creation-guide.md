...
```markdown
For the versioning model, see [`_meta/versioning-pattern/`](../../_meta/versioning-pattern/).
```
...
- ❌ Skipping `_template/` — building from scratch tends to drift from convention
...
- ❌ Re-deriving meta-patterns instead of referencing `_meta/`
