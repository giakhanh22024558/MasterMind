"""
Scaffold a new skill by copying the `core/template/` starter
...
WHAT IT DOES:
  1. Copies `core/template/` to `models/<new-skill-name>/`
...
DEST_ROOT = REPO_ROOT / "models"                  # concrete skills live under models/model_NNN/<diagram|document>/
TEMPLATE_DIR = REPO_ROOT / "core" / "template"
...
def main():
    ...
    new_skill_dir = DEST_ROOT / NEW_SKILL_NAME
    ...
    print(f"2. Define your skill's conventions in {new_skill_dir / 'conventions-schema'}")
    print(f"3. Provide sensible defaults in {new_skill_dir / 'conventions-defaults'}")
    print(f"4. Add at least one pattern in {new_skill_dir / 'patterns'}")
    print(f"5. Add at least one worked walkthrough in {new_skill_dir / 'examples'}")
    print("6. Update the relevant README ('Currently implemented' table)")
    print(f"7. (Optional) Add scripts in {new_skill_dir / 'scripts'}")
    print()
    print("Reference meta-patterns from `core/meta/` for:")
    print("  - Conventions as data: core/meta/conventions-as-data-pattern/")
    print("  - Defer-then-promote: core/meta/defer-then-promote-pattern/")
    print("  - Atomic edits: core/meta/atomic-edits-pattern/")
