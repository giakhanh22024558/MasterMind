...
Each skill that manipulates sync-prone files should provide reusable templates in `<skill>/scripts/` (or in `<skill>/_shared/scripts/` for multi-sub-skill). Examples (from `diagram/`):
