...
- **Pilot (`.xlsx`)** — generated by [`scripts/ba_md_to_xlsx.py`](../scripts/) into `output/`, with: `Priority` as a dropdown, `In Scope` as a project-value dropdown, `Ready?` / `Done?` as `☐`/`☑` checkbox-style dropdowns, and feature-level cells merged across each feature's rows.
